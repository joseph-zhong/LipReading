...
def _get_checkpoint_filepath(dataset_dir, num):
    return os.path.join(dataset_dir, "checkpoint_{}.pth".format(continue_from))
...
def _load_or_create_model(
    ...
    if continue_from:
        continue_from = _get_checkpoint_filepath(_util.getRelWeightsPath(dataset), continue_from)
    ...
    if cuda:
        model.cuda()
    return labels, model, optimizer, (avg_loss, start_iter, start_epoch)

def _init_results(epochs):
    loss_results = torch.Tensor(epochs)
    cer_results = torch.Tensor(epochs)
    wer_results = torch.Tensor(epochs)
    return loss_results, cer_results, wer_results
...
def _get_datasets(dataset_dir, train_split, labels, batch, num_workers):
    ...
    train_dataset = FrameCaptionDataset(videos[:split_idx], labels=labels)
    test_dataset = FrameCaptionDataset(videos[split_idx:], labels=labels)
...
def train(
  ...
  """
    ...
    labels, model, optimizer, (avg_loss, start_iter, start_epoch) = _load_or_create_model(
        dataset, continue_from, learning_rate, rnn_type, hidden_size, hidden_layers, momentum, cuda, tensorboard_writer)

    (train_dataset, train_loader), (test_dataset, test_loader) = _get_datasets(dataset_dir, train_split, labels, batch, num_workers)

    loss_results, cer_results, wer_results = _init_results(epochs)
    best_wer = None
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            data_time.update(time() - batch_start)
            ...
            avg_loss += loss_value
            losses.update(loss_valie, inputs.size(0))
        ...
        model.eval()
        with torch.no_grad():
            for i, (data) in _tqdm(enumerate(test_loader), total=len(test_loader)):
                inputs, targets, input_percentages, target_sizes = data
                input_sizes = input_percentages.mul_(int(inputs.size(3))).int()
            ...
            wer_results[epoch] = 100 * total_wer / len(test_loader)  # .dataset?
            cer_results[epoch] = 100 * total_cer / len(test_loader)
            ...
            if best_wer is None or best_wer > wer_results[epoch]:
                ...
                if os.path.isfile(weights_path):
                    copyfile(weights_path, model_path)
                else:
                    torch.save(
                        _model.DeepSpeech.serialize(model, optimizer=optimizer,
                            epoch=epoch, loss_results=loss_results, wer_results=wer_results,
